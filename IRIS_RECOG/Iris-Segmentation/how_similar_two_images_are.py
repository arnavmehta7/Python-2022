...
path = './001/L/%s' % (file_name) + '.jpg'
image_read = cv2.imread(path,0)
image_read = cv2.resize(image_read,(400,400))
original = image_read
for file in directory:
    print(file)
file_name = input("Filename with extension: ")
path = os.getcwd() + '/001/L/%s' % (file_name) + '.jpg'
image_read = cv2.imread(path,0)
image_read = cv2.resize(image_read,(400,400))

image_to_compare = image_read

print('Image has been readed ')
